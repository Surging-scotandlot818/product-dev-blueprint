...
export function generateTechSpec(p: Project): string {
  const out: string[] = [
    ...
    `| Layer | Choice |`,
    `|---|---|`,
    `| Frontend | ${fallback(p.dataTech.frontend)} |`,
    `| Backend | ${fallback(p.dataTech.backend)} |`,
    `| Database | ${fallback(p.dataTech.database)} |`,
    `| Auth | ${fallback(p.dataTech.authStrategy)} |`,
    `| CI/CD | ${fallback(p.dataTech.cicd)} |`,
    `| Hosting | ${fallback(p.dataTech.hosting)} |`,
