    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="flex items-start justify-between gap-6">
        <div className="min-w-0">
          <Link href="/projects" className="text-xs text-ink-500 hover:text-ink-800">← All projects</Link>
          <h1 className="text-3xl font-semibold tracking-tight mt-1">{project.name}</h1>
          <p className="text-ink-700 mt-1 max-w-2xl">{project.oneLiner || "No one-liner yet."}</p>
        </div>
      ...
      <Card className="mt-8 p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="text-sm font-medium text-ink-900">Intake progress</div>
            <div className="text-xs text-ink-500 mt-0.5">{completed} of {DOMAIN_ORDER.length} domains complete</div>
          </div>
      ...
      <h2 className="text-lg font-semibold mt-10 mb-3">Domains</h2>
        ...
        {DOMAIN_ORDER.map((d) => {
          ...
          return (
            <button
              ...
              className="text-left border border-ink-200 bg-white rounded-lg p-4 hover:border-ink-300"
            >
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium text-ink-900">{DOMAIN_LABEL[d]}</div>
                <Badge tone={status === "complete" ? "good" : status === "in-progress" ? "warn" : "neutral"}>
                  {status === "complete" ? "Complete" : status === "in-progress" ? "Draft" : "Empty"}
                </Badge>
              </div>
              <div className="text-xs text-ink-600 mt-1.5 leading-relaxed">{DOMAIN_BLURB[d]}</div>
            </button>
          );
        })}
      </div>
